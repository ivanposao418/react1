...
│   ├── flow-app1.txt
│   ├── flow-cleanup-app2.txt
│   ├── hook kako radi.md
│   └── objasnjenja-app1withhook.md
├── hooks
...
function App() {
  ...
  //const debouncedQuery = useDebouncedValue(query, 2500);   
  const debouncedQuery = useDebouncedValue(query, 2500);
...
`explanations/explanations-app2.md`:
...
query updates immediately
   ↓
debounce effect waits
   ↓
debouncedQuery updates AFTER pause



```

`explanations/flow-cleanup-app2.txt`:

```txt


## 🧭 FLOW CHART (this is the money part)
...
Cleanup is **not immediate**, it’s **reactive**.

---
```

`explanations/hook kako radi.md`:
...
`explanations/objasnjenja-app1withhook.md`:
...
```md
primjecujem da mi je puno lakse kad 

CHUNKING PREPISIVANJE
1. kod prepisivanja koda odredim dio po dio mali 
    makni fokus sa svega drugog samo na mali dio 
recimo samo fokus na if , pa na state setting i sl.
